...
export const createWindowHTML = myWindow => {
  ...
  let collapseId = StringBuilder.buildWindowBodyParentId(myWindow)
  ...
  <div class="input-group collapse" id="${StringBuilder.buildWindowEditInputGroupId(myWindow)}">
    <input id="${StringBuilder.buildWindowEditInputId(myWindow)}" type="text" class="form-control" placeholder="New window name" aria-label="New window name" aria-describedby="${editTitleOkId}">
    <button class="btn btn-outline-secondary" type="button" id="${editTitleOkId}" style="color: white;">Ok</button>
  </div>
  <button id="${StringBuilder.buildWindowCollapseButtonId(myWindow)}" class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#${collapseId}" aria-expanded="true" aria-controls="${collapseId}">
